  ...
  const handleSocialSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (socialModal === 'google') {
        await loginWithGoogle(socialEmail, socialName);
      } else if (socialModal === 'microsoft') {
        await loginWithMicrosoft(socialEmail, socialName);
      }
      setSocialModal(null);
    } catch (err: any) {
      setError('Falha ao conectar conta social.');
    } finally {
      setIsSubmitting(false);
    }
  };
            ...
            <div className="p-6 sm:p-8 rounded-3xl bg-[#040a1c]/90 border border-cyan-500/30 backdrop-blur-2xl shadow-[0_0_50px_rgba(0,0,0,0.8)] relative">
              ...
              <div className="space-y-2 mb-6">
                <button
                  ...
                  disabled={isLoading}
                  className="w-full py-2.5 px-4 rounded-xl bg-slate-900 border border-slate-700 hover:border-cyan-400 text-slate-200 font-medium text-xs flex items-center justify-center gap-3 transition-all hover:bg-slate-800 shadow-md group"
                >
                  ...
                  <span>Entrar com Meu Gmail / Google</span>
                </button>
                ...
                >
                  ...
                  <span>Entrar com Meu Microsoft / Outlook</span>
                </button>
              </div>

              {/* Social Login Interactive Popup */}
              {socialModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                  <div className="w-full max-w-sm bg-[#050b1e] border border-cyan-500/40 rounded-3xl p-6 shadow-[0_0_50px_rgba(0,242,254,0.2)] animate-in fade-in zoom-in-95 duration-200">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="font-rajdhani font-bold text-lg text-white">
                          {socialModal === 'google' ? 'Conectar sua Conta Google' : 'Conectar sua Conta Microsoft'}
                        </h4>
                      </div>
                      <button 
                    ...
                    <form onSubmit={handleSocialSubmit} className="space-y-4">
                      ...
                      <div>
                        <label className="block text-[10px] font-mono text-slate-400 mb-1">
                          {socialModal === 'google' ? 'SEU E-MAIL GMAIL' : 'SEU E-MAIL OUTLOOK / HOTMAIL'}
                        </label>
                        <input
                          type="email"
                          required
                          ...
                          placeholder={socialModal === 'google' ? 'seu.nome@gmail.com' : 'seu.nome@outlook.com'}
                          className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white text-xs focus:outline-none focus:border-cyan-400"
                        />
                      </div>
                      ...
                      <div className="flex gap-2 pt-2">
                        <button
                          type="button"
                          onClick={() => setSocialModal(null)}
                          className="flex-1 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-rajdhani text-xs font-bold"
                        >
                          Cancelar
                        </button>
                        <button
                          ...
                          className="flex-1 py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-rajdhani text-xs font-bold uppercase tracking-wider"
                        >
                          {isSubmitting ? 'Conectando...' : 'Entrar com Minha Conta'}
                        </button>
                      </div>
                    </form>
                  </div>
